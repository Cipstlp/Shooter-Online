    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        public void HandleCollision(GameObject obj, Collision c)
        {
			...
			if (playerHealth != null&&playerHealth.currentHealth>0) {
                ...
                GameObject player = playerHealth.gameObject; //warning?
                if (playerHealth.GetComponent<PlayerID>().playerUniqueName != this.ownerName)
                {
                    CmdTellServerWhoGotShot(playerHealth.GetComponent<PlayerID>().playerUniqueName, 20);
